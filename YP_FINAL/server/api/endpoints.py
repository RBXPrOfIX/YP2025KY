...
async def get_lyrics_encrypted(
    request: Request,
...
async def find_similar_encrypted(
    ...
    try:
        ...
        src_genres = set(json.loads(source.genre) if source.genre else [])
        ...
        payload = json.dumps({"similar_tracks": sorted(results, key=lambda r: -r["similarity"])[:5]}, ensure_ascii=False).encode("utf-8")
        encrypted = encrypt_payload(payload)
        return {"data": encrypted}
